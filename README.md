...
The current room preset resolves the conference microphone using the ordered aliases `Logitech Webcam C925e|Audio Streaming`, because the room/Teams connection can expose the same intended input under either name. This avoids silently switching microphones when macOS renumbers devices or changes the advertised hardware label. Startup prints the resolved microphone name and capture number. Override it with `CC_WHISPER_CAPTURE_NAME="preferred name|fallback name" npm run room:preview`; use `CC_WHISPER_CAPTURE_ID` only as an explicit numeric override. The camera must see the student’s shoulders, elbow, wrist, and raised palm; a head-only crop cannot confirm the gesture.
...
- `headless/reasoning/tutor-provider.ts` — schema-constrained answer and comprehension-assessment adapter for local Ollama
