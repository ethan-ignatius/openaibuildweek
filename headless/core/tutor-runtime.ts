...
import { coachingFeedbackScene, decimalComparisonScene, ExcalidrawBoardController, genericTutorScene, tutorThinkingScene, type ExcalidrawScene } from "../whiteboard/excalidraw-tool";
...
export class TutorRuntime {
  ...
  private async handleHandRaise(event: HeadlessEvent) {
    ...
    if (this.processingQuestion || this.activeInteraction || this.activeGeneralCheck || this.activeSpokenText) {
  ...
  private async handleQuestion(event: HeadlessEvent) {
    ...
    if (this.tutorProvider) {
      ...
      try {
        ...
        if (generalCheck) await this.speak(generalCheck.check.prompt, "en");
